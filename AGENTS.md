...
3. A fuel-based definitional interpreter (`LeanModels/Python/Semantics.lean`) gives it meaning. Since the H1 core re-shape (docs/memory-model.md v2) the mutual block is `Run`-typed over `FrameState` (`World` = heap + globals + stdout, plus this frame's locals; state retained on `.ok` AND `.exn`), **`callIn` is the mutual-recursion point**, and the public `callFunction m f args fuel : Res Val` is a NON-recursive wrapper — thaw (`RVal.thawArgs`, kernel-reducible) ∘ fresh-world ∘ `callIn` ∘ deep-freeze (`Run.toPublic`) — with its signature and `CallsTo` unchanged. FROZEN recursion points: `callIn`/`execWhile`/`execFor` — unfold one step with `rw [callIn.eq_2]` / `rw [execWhile.eq_2]` / `rw [execFor.eq_2/eq_3]`; `callFunction` itself unfolds freely (`unfold callFunction`, and it is in `py_simp`'s default set). Tier (2026-08-06): `while`/`if`/`for`-over-lists/assignment/tuple-unpack; builtins `len`/`sorted`/`max`/`min`/`abs`/`int`; G1 constant module globals resolved from `Module.topLevel` (stage-1: `Val`-side accumulator, world-globals empty) — everything heap-shaped (`.ref` arms, dict literals, subscript stores) is loudly `unsupported` until H1-proper. The `for` proof pattern (frozen-tail list induction via `execFor_mono`, hand-unrolled first iteration) is `Examples/python/sf_bound_for/proof.lean`; `py_vcgen`/`py_loop` do NOT walk `for` loops yet — manual route only. Stage-1 facts the proof layer leans on: `worldInv` (nothing allocates ⇒ decided runs return their input world) and the thaw/freeze roundtrip (`RVal.freeze_thaw`/`RVal.eq_thaw_of_freeze`).
