...
4. Theorems live in `# lean[ … # ]` comment blocks inside the `.py` file, spliced verbatim into a generated companion `Examples/<Name>.lean` — except in the **three-file layout** (`Examples/tri/`, `Examples/gcd/`): pure `.py` (envelope only, no companion), hand-written `spec.lean` (checks + theorem statements, each `:= by proofs`) and `proof.lean` (the real proofs; namespace = its module path). The `proofs` tactic docstring in [Surface.lean](LeanModels/Python/Surface.lean) is the contract; statement duplication between the two files is by design and typechecked by the `:= by proofs` reference.
...
| loop-free body with **two sequential `if`s** (`py_prove`'s one-round split recipe fails — failure table below) | `refine ⟨32, ?_⟩`, `by_cases` each branch condition, `py_simp [callFunction, f, h1, h2] <;> grind` — see `Examples/python/ag_clamp01.py` |
...
- Never edit generated files: `Examples/*.lean` companions (header says `AUTOGENERATED … DO NOT EDIT`) and `*.json` envelopes anywhere. Edit the `.py`, re-extract. (Three-file `spec.lean`/`proof.lean` carry no AUTOGENERATED header and are edited directly.)
