...
3. A fuel-based definitional interpreter (`LeanModels/Python/Semantics.lean`) gives it meaning: `callFunction m f args fuel : Res Val` with `ok / exn / timeout / unsupported`. Tier (2026-08-06): `while`/`if`/`for`-over-lists/assignment/tuple-unpack; builtins `len`/`sorted`/`max`/`min`/`abs`/`int`; G1 constant module globals (`MATE_UPPER = 69290`, tuple-unpack constants) resolved from `Module.topLevel` — out-of-tier module bindings and post-`import`/`ClassDef` name misses are loudly `unsupported`. `execFor` is a FROZEN recursion point exactly like `callFunction`/`execWhile`: unfold one step with `rw [execFor.eq_2]` (nil) / `rw [execFor.eq_3]` (cons); the `for`-loop proof pattern (frozen-tail list induction in fuel-threshold form via `execFor_mono`, hand-unrolled first iteration because the target variable is created mid-loop) is `Examples/python/sf_bound_for/proof.lean`. `py_vcgen`/`py_loop` do NOT walk `for` loops yet — manual route only.
...
| recursive function | induction on the **math argument** (`Nat.strongRecOn`), base cases `py_prove`; step: `py_lift ⟨f₁, h₁⟩ := ih … with [f]` per IH, then `refine ⟨f₁ + f₂ + 32, ?_⟩`, `rw [callFunction.eq_2]`, `py_simp [f, facts…]`, `simp (disch := omega) only [h₁, h₂]` — see `Examples/python/fib/proof.lean` |
