/-
Examples/ag_clamp01 — three-file example layout (see Examples/tri/spec.lean
...
(the real proofs, namespace `Examples.ag_clamp01.proof`).
...
-/
import Examples.ag_clamp01.proof
...
`Examples/ag_clamp01/proof.lean`. (Not `@[spec]`: the ∃-fuel arrow is not
a Hoare-triple/simp shape — see Examples/add/add.py.) -/
