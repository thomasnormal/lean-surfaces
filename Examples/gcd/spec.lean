/-
Examples/gcd — three-file example layout (see Examples/tri/spec.lean for
the pattern rationale): gcd.py (pure Python), gcd.json (generated
envelope), THIS FILE (statements, `:= by proofs`), proof.lean (the real
proofs, namespace `Examples.gcd.proof`).
-/
import Examples.gcd.proof
...
`Examples/gcd/proof.lean`. (Not `@[spec]`: the ∃-fuel arrow is not a
Hoare-triple/simp shape — see `Examples/add/add.py`.) -/
