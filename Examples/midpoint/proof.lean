/-
Proof module for `Examples/midpoint/spec.lean` (three-file example
layout). Every theorem stated in spec.lean is proved here under the same
name; the spec side is `:= by proofs`, which resolves
`Examples.midpoint.proof.<decl>` (Surface.lean). Statements are duplicated
...
namespace Examples.midpoint.proof
...
the not-load-bearing `ha`/`hb`: `Examples/midpoint/spec.lean`). -/
