/-
Examples/my_abs — three-file example layout (see Examples/tri/spec.lean
for the pattern rationale): my_abs.py (pure Python), my_abs.json
(generated envelope), THIS FILE (statements, `:= by proofs`), proof.lean
(the real proofs, namespace `Examples.my_abs.proof`).
-/
import Examples.my_abs.proof
...
`py_prove` handles the branch itself (`Examples/my_abs/proof.lean`): its
...
shapes; the ∃-fuel arrow is neither — see Examples/add/add.py.) -/
