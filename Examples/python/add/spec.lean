/-
Examples/python/add — three-file example layout (see Examples/python/tri/spec.lean for
the pattern rationale): add.py (pure Python), add.json (generated
envelope), THIS FILE (statements, `:= by proofs`), proof.lean (the real
proofs, namespace `Examples.python.add.proof`).
-/
...
(`Examples/python/add/proof.lean`). (Not `@[spec]`: that attribute takes
