/-
Proof module for `Examples/python/ag_clamp01/spec.lean` (three-file example
layout). Every theorem stated in spec.lean is proved here under the same
name; the spec side is `:= by proofs`, which resolves
`Examples.python.ag_clamp01.proof.<decl>` (Surface.lean). Statements are
...
namespace Examples.python.ag_clamp01.proof
...
end Examples.python.ag_clamp01.proof
