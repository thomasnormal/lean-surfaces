/-
Proof module for `Examples/python/arith/spec.lean` (three-file example layout).
Every theorem stated in spec.lean is proved here under the same name; the
spec side is `:= by proofs`, which resolves `Examples.python.arith.proof.<decl>`
...
namespace Examples.python.arith.proof

open LeanModels LeanModels.Python

load_program arith from "Examples/python/arith/arith.json"
...
end Examples.python.arith.proof
