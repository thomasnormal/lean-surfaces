/-
Examples/python/rsa_inverse — three-file example layout (see Examples/python/tri/spec.lean
...
(the real proofs, namespace `Examples.python.rsa_inverse.proof`).
...
`unsupported` row).
-/
import Examples.python.rsa_inverse.proof
...
simp shapes (cf. Examples/python/add/spec.lean). -/
