/-
Examples/system-verilog/adder — the three-file example layout (SV lane):
  ...
  proof.lean    — the real proofs (namespace `Examples.«system-verilog».adder.proof`)
...
-/
import Examples.«system-verilog».adder.proof
import LeanModels.Sv.Tests
import LeanModels

open LeanModels.Sv
open Examples.«system-verilog».adder.proof (adderDesign adderTrace)
...
#eval show IO Unit from do
  let d ← EnvelopeIngest.loadFile "Examples/system-verilog/adder/adder.sv.json"
  unless d == adderDesign do
    throw (IO.userError "Examples/system-verilog/adder/adder.sv.json ≠ adderDesign")
...
`Examples/system-verilog/adder/proof.lean`. -/
