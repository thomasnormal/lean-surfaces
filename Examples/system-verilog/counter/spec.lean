/-
Examples/system-verilog/counter — the three-file example layout (SV lane):
  ...
  proof.lean      — the real proofs (namespace `Examples.«system-verilog».counter.proof`)
...
open LeanModels.Sv
open Examples.«system-verilog».counter.proof (counterDesign counterModel counterTrace)
...
#eval show IO Unit from do
  let d ← EnvelopeIngest.loadFile "Examples/system-verilog/counter/counter.sv.json"
  unless d == counterDesign do
    throw (IO.userError "Examples/system-verilog/counter/counter.sv.json ≠ counterDesign")
