/-
Examples/system-verilog/race_blk — the three-file example layout (SV lane):
  ...
  proof.lean       — the real proofs (namespace `Examples.«system-verilog».race_blk.proof`)
...
-/
import Examples.«system-verilog».race_blk.proof
import LeanModels.Sv.Tests
import LeanModels

open LeanModels.Sv
open Examples.«system-verilog».race_blk.proof (raceBlkDesign raceStim)
...
#eval show IO Unit from do
  let d ← EnvelopeIngest.loadFile "Examples/system-verilog/race_blk/race_blk.sv.json"
  unless d == raceBlkDesign do
    throw (IO.userError "Examples/system-verilog/race_blk/race_blk.sv.json ≠ raceBlkDesign")
