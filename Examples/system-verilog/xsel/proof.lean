/-
Proof module for `Examples/system-verilog/xsel/spec.lean` (three-file example layout,
...
The rest is the house architecture (`Examples/system-verilog/adder/proof.lean`): one
...
equal to `Examples/system-verilog/xsel/xsel.sv.json`.
-/
import LeanModels.Sv.ToggleExample

namespace Examples.«system-verilog».xsel.proof

open LeanModels.Sv

/-- `Examples/system-verilog/xsel/xsel.sv` (`always_comb` if/else mux on `sel`),
hand-transcribed. -/
...
Same shape as `Examples/system-verilog/adder/proof.lean`: one pass computes the mux, the
...
end Examples.«system-verilog».xsel.proof
