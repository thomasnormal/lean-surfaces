/-
Proof module for `Examples/tut_04/spec.lean` (three-file example layout).
Every theorem stated in spec.lean is proved here under the same name; the
spec side is `:= by proofs`, which resolves `Examples.tut_04.proof.<decl>`
...
fib pattern — see Examples/fib/proof.lean): the twin statements must
