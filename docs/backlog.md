...
drifts**.
## L54 — THE C23 GOAL: the suite census, and rung 0 already clears 83% (2026-08-22)

Thomas ruled the endgame — **option (c)**, the general C completeness ladder
([docs/c-tier-charter.md](c-tier-charter.md) §3) — with a measurable
definition: *"Is there a standard test suite for C23? We should use that as
the goal."* There is no official ISO suite (ISO publishes the standard, not a
corpus), so the goal is restated as: **the Lean semantics, RUN on suite
programs, reproduces the suites' expected behavior**, scored
MATCH/REFUSE/DIVERGE with the zero-DIVERGE invariant. The charter is
[docs/c23-goal.md](c23-goal.md); the measurements are in
`docs/c23-suite-census.json`, taken by `harness/c_suite_census.py` on pinned
revisions. ctwin stays rung 0; the twin-bridge stays a later option.

**Five corpora, censused at pinned revisions.** `reachable` = parses under the
pinned profile, writes nothing outside rung 0's 45-kind vocabulary, needs no
hosted header.

| corpus | tests | censused | parsed | reachable | oracle |
| --- | ---: | ---: | ---: | ---: | --- |
| c-testsuite | 220 | 220 | 219 | **147** | expected stdout |
| gcc.c-torture/execute | 1918 | 300 | 246 | **196** | exit status |
| gcc c23-* (dg-do run) | 76 | 76 | 48 | **11** | exit status |
| clang test/C/C23 | 37 | 37 | 12 | **3** | diagnostics, not behavior |
| fujitsu/C | 37191 | 300 | 300 | **0** | expected stdout |

**THE MEASUREMENT THAT MOVED EVERY NUMBER: written vs synthesized.** The raw
out-of-vocabulary counts were dominated by attribute nodes — `BuiltinAttr`
275, `NoThrowAttr` 267, `NoInlineAttr` 52. **No test wrote them.** Measured:
`gcc.c-torture/execute/20000313-1.c` contains `void abort (void);` and **zero**
occurrences of `__attribute__` or `__builtin`, yet its AST carries both —
clang synthesizes them on declarations of names it knows as builtins. With the
compiler's own bookkeeping set aside the torture suite goes from **7 in-vocab
to 206, a 29x correction**, and from "unreachable corpus" to "the second-best
rung-1 target we have". The instrument reports both numbers and says which is
which.

**RUNG 0 ALREADY CLEARS 357 OF 431 FREESTANDING PARSED TESTS — 83% — before
anything is added.** That was not predictable: the flagship corpus's
*accidental* vocabulary turns out to be a good approximation of what real C
test programs use. The greedy ladder above it is SHORT: `SwitchStmt`+`CaseStmt`
→ 361, `ImplicitValueInitExpr` → 366, `NullStmt` → 374, `DefaultStmt` → 383,
`VAArgExpr` → 393, `StaticAssertDecl` → 398, `DesignatedInitUpdateExpr` → 403.
Eight constructs, 357 → 403. The order is regenerated by the instrument, not
transcribed.

**TWO VERDICT STYLES, and one needs no output modeling at all.** c-testsuite
and Fujitsu are expected-stdout. GCC's torture and c23-run are **exit status**:
`abort()` on failure, fall off `main` on success (torture's top calls are
`abort` 162, `exit` 95, `__builtin_abort` 72 of 246 parsed). The architecture
memo §6 already makes `abort` *"a distinguished TERMINAL outcome, not a
refusal"* — so **the largest reachable corpus is scoreable against machinery
the charter already committed to, with zero output modeling.** That is rung 1.

**THE LIBC SURFACE IS NOT "LIBC" — IT IS `printf`.** Measured distinct
functions actually CALLED: c-testsuite 26, with **149 of 220 tests calling none
at all** and `printf` the only common one (68); Fujitsu `printf` 276 / `puts`
24; the GCC corpora `abort`/`exit`. `printf` plus `puts` plus the already-
specified `abort`/`exit` covers the overwhelming majority of all five.

**The preprocessor question, MEASURED: it costs nothing.** The suites do not
assume a preprocessed TU — c-testsuite tags 96 of 220 `needs-cpp`, Fujitsu 283
of 300 by this census's narrower measure. But translation phases 1-6 already
run OUTSIDE Lean in clang (memo §4), so `needs-cpp` tests arrive pre-expanded
and are scored on their expansion. A decision taken for other reasons pays off
here.

**LICENSES — and c-testsuite is a TRAP.** Its top-level `LICENSE` is MIT and
says in its own first two lines that it covers *"all testing software, but not
for individual test cases"*; `tests/LICENSE` defers to each case's `.otag`.
Reading all 219: **150 cases originate in `scc` (ISC), 69 in `tinycc`
(LGPL-2.1)**. A lane that read only the top-level MIT and vendored the
directory would have redistributed LGPL code under a wrong notice. GCC's
testsuite carries **no per-file header** (measured: 0 of the sampled torture
files) and inherits GPLv3+. clang and Fujitsu are Apache-2.0 WITH
LLVM-exception. **Recommendation: fetch at test time, pin by revision, vendor
nothing** — the only uniformly correct policy, and one the lane already
follows (`Examples/c/sunfish/` holds an envelope and no `.c`).

**Also measured**: clang's C23 directory is feature bookkeeping, not behavior —
58 `-verify` / 17 `-fsyntax-only` against 6 `-emit-llvm`, with 37 tests mapping
to 28 distinct WG14 N-papers by filename. Useful as a free per-paper checklist,
almost useless as an execution corpus. And the low `parsed` rates (48/76,
12/37, 246/300) are dg/lit tests wanting harness options or *designed* to be
rejected — a category, not a failure, and never counted as a small program.

**The scoreboard is SPECCED, NOT BUILT.** `harness/c_refusal_census.py` has
nothing to run until M2's interpreter exists; an instrument with no subject is
a stub. Its spec is §4.2: one runner process for the batch, one row per test in
test order, REFUSE broken out by its three causes (out-of-tier / UB-refused /
unmodeled-libc — they retire on completely different schedules, and UB-refused
NEVER retires because it is the product), non-zero exit only on DIVERGE, a
`runner-error` row rather than a missing row, and no whitelist for silencing a
mismatch.

**DECISION POINTS BACK TO THOMAS** (§5): (1) commercial suite purchase y/n —
Plum Hall CV-Suite and Solid Sands SuperTest add systematic per-clause coverage
instead of regression accretion, but cannot be vendored or likely even quoted,
so they would be a private scoreboard beside the public one; **nothing was
purchased or proposed**. (2) vendored vs fetched — recommendation is fetched,
uniformly. (3) the float modeling approach when its rung arrives — the Fujitsu
sample is math-heavy and `docs/c-profile.md` already records that **neither
host defines `__STDC_IEC_60559_BFP__`**, so a float rung would differential-test
against a contract neither side signed; R4's gate stands.

**Honest scale**: this is scoped to the LANGUAGE, not clause 7; high scores
would mean "agrees with what these projects test", NOT "conforms to ISO
9899:2024"; and two corpora are sampled at 300, with the Fujitsu sample the
first directory by path and math-heavy, so a stratified sample is owed before
any Fujitsu number is used for pricing.

### Triad

`lake build` **3692 jobs green**; `docs_check` **73/73**, 15
illustrative-exempt; `diff_test` **1394 cases, 0 failed, 118 whitelisted, 1276
matched**; `script_corpus` **65 scripts, 0 failed, 50 matched, 15 loud**. This
landing is research + two instruments + docs; **no Lean changed**, so no axioms
