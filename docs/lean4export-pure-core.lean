...
/-!
FORK DIVERGENCE — lean-surfaces, 2026-08-23.  Upstream base: `af5aa64`.

The exporter is factored into a PURE CORE plus a thin IO wrapper, so the
round-trip property `parse (dump E) = E` is STATABLE.  Against upstream it is
not: `M`'s base was `IO` and emission went through `IO.println`, an opaque
`@[extern]` primitive a Lean proof cannot evaluate through.

THE DIVERGENCE IS EXACTLY FOUR EDITS:
  1. `State.out`          -- accumulated output lines
  2. `M`'s base           -- `IO` becomes `Id`
  3. three emission sites -- `IO.println x` becomes `emitLine x`
                             (`getIdx`, `dumpObj`, `dumpMetadata`)
  4. `M.run`              -- the thin wrapper: run the pure core, then flush

BEHAVIOUR IS IDENTICAL: the same lines in the same order reach stdout, because
`emitLine` appends where `IO.println` printed and `M.run` flushes in order.  The
gate for that claim is upstream's own 22 `#guard_msgs` in `Test.lean`, which
record the exact printed NDJSON — they pass unchanged or this refactor is wrong.

TRIPWIRE: `harness/lean_export_manifest.py --check-divergence` re-reads the
upstream touch points at the pinned base and REFUSES if they have moved, so this
fork goes loud rather than stale when upstream edits them.

MORE UPSTREAMABLE, NOT LESS: a pure core makes the exporter unit-testable as well
as provable — `M.runPure` returns the lines instead of printing them.  No
upstream contact has been made; recorded as a potential offer artifact only.
-/
...
structure State where
  ...
  /-- FORK: accumulated output lines.  Upstream printed directly; the pure core
  appends here and `M.run` flushes, which is what makes the exporter's result a
  VALUE a theorem can talk about. -/
  out : Array String := #[]

/-- FORK: base monad is `Id`, not `IO` — the whole point of the refactor.  Every
exporter function keeps its `M α` signature unchanged. -/
abbrev M := ReaderT Context <| StateT State Id

/-- FORK: replaces `IO.println` at the three emission sites. -/
def emitLine (s : String) : M Unit :=
  modify fun st => { st with out := st.out.push s }

/-- FORK: the PURE CORE.  Returns the value and the emitted lines instead of
printing them — this is the function the round-trip theorem is about. -/
def M.runPure (env : Environment) (act : M α) : α × Array String :=
  let (a, st) := Id.run <| StateT.run (s := {}) <| ReaderT.run (r := { env }) act
  (a, st.out)

/-- The thin IO wrapper.  Behaviourally identical to upstream's `M.run`: the same
lines in the same order reach stdout. -/
def M.run (env : Environment) (act : M α) : IO α := do
  let (a, lines) := M.runPure env act
...
def getIdx [Hashable α] [BEq α] (x : α) (namespaced: String) (getM : State → HashMap α Nat) (setM : State → HashMap α Nat → State) (rec : M Json) : M Nat := do
  ...
  let m ← getM <$> get
  let idx := m.size
  emitLine (s.setObjVal! namespaced idx).compress
...
where
  ...
  dumpObj (fields : List (String × Json)) : M Unit :=
    emitLine <| Json.mkObj fields |>.compress
...
def dumpMetadata : M Unit := do
  emitLine exportMetadata.compress
