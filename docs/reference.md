...
| `lake exe leanmodels-run --batch <jobs.jsonl> [--fuel N]` | one process, one job line per row (`{"path":…,"function":…,"args":[…],"fuel":N?}`), one canonical result line per job in order, flushed per line; envelopes cached by path; unexecutable jobs emit `runner-error` lines + nonzero exit |
