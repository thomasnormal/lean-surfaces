...
| `py_vcgen [prog] (inv := …) (dec := …) …` (py_vcgen layer 3, [VCTactic.lean](../LeanModels/Python/VCTactic.lean)) | the VC-generating walker: bridges a `==>`/`⇓`, `PyTriple`, or `∃ v, CallsTo … v ∧ Φ v` goal to the function-body triple and walks it — straight-line auto-discharged, i-th `while` from the i-th `inv`/`dec` clause pair (plus optional `exit<i>` for `break`-carrying loops; clauses omitted → delayed `inv<i>`/`dec<i>` goals, plus `exit<i>` when the loop carries a `break`), calls from local `CallsTo` hypotheses or `@[py_spec]` | residual goals are pure math over named atoms (`hinv1…`, `hcont`, `hif`, primed exit values), tagged `init`/`preserve`/`dec`/`exit`/`ret`/`err`/`side` (same-tag duplicates numbered `preserve`, `preserve2`, …); module docstring is the contract |
...
|---|---|---|---|
| `py_prove` | `py_prove [prog, extras…]` | total goals `f(…) ==> v` and `f(…) ==>! e` for **loop-free** bodies, straight-line or branching (fuel witness 32, symbolic execution, `split`/`omega` mop-up) | clean arithmetic residuals on partial success; fails with a curated pointer to `py_vcgen`/`py_simp`/`py_threshold` when the body leaves interpreter state (loops); recursion wants `py_lift` |
...
| `Examples/python/nested_flow/` | the control-flow stress example: nested `while`s, `break` inside an `if`, mid-loop `return` — proved by `py_vcgen` from two clause pairs + one `exit2` fact against mathlib's `Nat.minFac` |
