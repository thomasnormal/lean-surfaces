/-
roundQ INCH — THE DECLARATIVE SPEC FIRST, because it is the half that carries
the weight.

WHY DECLARATIVE FIRST.  charter §3.5.2 warns that a downstream proof must target
ROUND-OF-EXACT and never our bit algorithm: *"these output bits equal what
`UnpackedFloat.div` computes" is a tautology about an implementation.*  A
COMPUTABLE `roundQ` cannot escape that on its own — any correct implementation
will structurally resemble core's, so `op fmt x y = roundQ fmt (exact …)` proved
against it would be close to circular.

The escape is the family's own spec/interpreter split one level down (charter
§3.3): a DECLARATIVE `IsCorrectlyRounded` that mentions no algorithm at all —
core's or ours — and says only what IEEE says: *the representable value nearest
the exact one, ties to even*.  That predicate is what `op_correct` should be
stated against; the computable `roundQ` is then proved to satisfy it, and its
resemblance to core stops mattering.

This probe fixes that predicate's statement and checks it elaborates and has
the properties it must have.  No `Float`, no core operation, ℝ nowhere.

RUN:  tools/check.sh harness/softfloat/probe_roundq.lean
-/
import Init.Data.Float.Model

open Float.Model

/-! ## The exact-value algebra this needs (mirrors LeanModels/SoftFloat/Basic.lean) -/
...
/-- Exact equality: cross-multiplication, so `1/2 = 2/4`. -/
protected def Eq (a b : Q) : Prop := a.num * b.den = b.num * a.den
/-- Exact `≤`, by cross-multiplication.  Both denominators are positive. -/
...
/-- `|a - b|` — the distance between two exact values. -/
...
#print axioms nearest_of_exact
#print axioms directed_of_exact
